"""Communication settings panel for the KO II GUI."""
...
def apply_communication_panel_patch(gui_module: Any) -> None:
    """Add a compact in-app communication settings panel."""

    app_class = gui_module.KO2DawApp
    if getattr(app_class, "_communication_panel_patch_installed", False):
        return
    ...
    def _build_menu(self) -> None:
        ...
        comm_menu.add_command(label="Communication Panel", command=self._show_communication_panel)
        comm_menu.add_command(label="Communication Summary", command=self._show_communication_summary)
        menu.add_cascade(label="Communication", menu=comm_menu)

    def _show_communication_panel(self) -> None:
        if getattr(self, "communication_window", None) and self.communication_window.winfo_exists():
            self.communication_window.lift()
            return
        ...
        for row, (label, variable) in enumerate(options, start=1):
            check = tk.Checkbutton(body, text=label, variable=variable, bg="#d8d4c8", anchor="w")
        ...
        fields = (
            ...
            ("Advanced phrase", self.comm_phrase, "*"),
        )
        for offset, (label, variable, mask) in enumerate(fields, start=5):
            tk.Label(body, text=label, bg="#d8d4c8").grid(row=offset, column=0, sticky="w", pady=3)
        ...
        summary.insert(tk.END, self._communication_summary_text())
        summary.configure(state=tk.DISABLED)

        actions = tk.Frame(body, bg="#d8d4c8")
        actions.grid(row=12, column=0, columnspan=2, sticky="ew")
        tk.Button(actions, text="APPLY", command=self._apply_communication_panel).pack(side=tk.LEFT, padx=3)
        tk.Button(actions, text="SAVE", command=self._save_communication_panel).pack(side=tk.LEFT, padx=3)
        tk.Button(actions, text="PROTOCOL", command=self._show_protocol_window).pack(side=tk.LEFT, padx=3)
        tk.Button(actions, text="CLOSE", command=win.destroy).pack(side=tk.RIGHT, padx=3)

    def _apply_communication_panel(self) -> bool:
        ...
        if applied:
            self._record_communication_event("applied")
        return applied

    def _save_communication_panel(self) -> None:
        if getattr(self, "communication_window", None) and self.communication_window.winfo_exists():
            if not self._apply_communication_panel():
                return
        path = save_app_settings(self.settings_path, self.app_settings)
        self._record_communication_event("saved")
    ...
    def _apply_settings(self, *, show_message: bool = True) -> bool:
        ...
        if result:
            self._record_communication_event("settings")
        return result

    def _show_communication_summary(self) -> None:
        messagebox.showinfo("KO II Communication", self._communication_summary_text())
    ...
    def _communication_summary_text(self) -> str:
        ...
        return "\n".join(
            [
                ...
                f"Advanced armed: {'yes' if settings.write_enabled else 'no'}",
                f"Timeout: {settings.sysex_timeout_sec}s",
                f"Message limit: {settings.max_sysex_bytes} bytes",
                f"Scan: {settings.scan_pages_per_dir} pages, depth {settings.scan_max_depth}, dirs {settings.scan_max_dirs}",
                "",
                "Use Protocol Monitor for full app/device message visibility.",
            ]
        )

    def _record_communication_event(self, label: str) -> None:
        detail = self._communication_summary_text()
        self._log(f"communication {label}: {self.app_settings.access_mode}")
        if hasattr(self, "_record_protocol"):
            self._record_protocol("app", f"communication-{label}", self.app_settings.access_mode, detail)

    def _send_device_file_playback(self, action: int) -> None:
        if not self.app_settings.playback_enabled:
            self._record_communication_event("preview-blocked")
    ...
    app_class._show_communication_panel = _show_communication_panel
    app_class._apply_communication_panel = _apply_communication_panel
    app_class._save_communication_panel = _save_communication_panel
    app_class._apply_settings = _apply_settings
    app_class._show_communication_summary = _show_communication_summary
    app_class._communication_summary_text = _communication_summary_text
    app_class._record_communication_event = _record_communication_event
    app_class._send_device_file_playback = _send_device_file_playback
    app_class._communication_panel_patch_installed = True
