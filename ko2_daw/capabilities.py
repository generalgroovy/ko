    ...
    def to_text(self) -> str:
        ...
        lines.extend(
            [
                ...
                "- Full read-only snapshots preserve node/path/size metadata and a canonical SHA-256 inventory hash for before/after integrity checks.",
                "- Selected PCM sounds can be downloaded page-by-page, checked against metadata CRC32, stored by SHA-256, converted to WAV, and previewed locally.",
                "- A file GET must be consumed to its declared length; the app blocks normal disconnect/close during active downloads.",
...
def _base_findings(sysex_ok: bool, file_ok: bool, midi_ok: bool) -> list[CapabilityFinding]:
    ...
    return [
        ...
        CapabilityFinding(
            ...
            "Use Hardware Files for browsing, Device Snapshot for inventory, and Device Library for verified PCM download/WAV preview.",
        ),
        CapabilityFinding(
            "Device File GET",
            "Download PCM data and paged JSON metadata",
            "confirmed",
            "Node 207 downloaded as 3,988 bytes over 13 pages; metadata CRC32 matched the raw PCM.",
            "Store immutable SHA-256 bundles, preserve raw bytes, and expose waveform/local preview.",
        ),
        CapabilityFinding(
            "Current State",
